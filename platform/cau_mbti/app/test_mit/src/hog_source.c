...
void hog_feature(unsigned char* img, double* dst, bool approx_mode ){
	...
	//double descriptor[HISTOGRAMSIZE];
	//double sum_of_block[N_BLOCK_ROW * N_BLOCK_COL];
	double * descriptor= (double *)malloc(sizeof(double)*HISTOGRAMSIZE);

	double * sum_of_block= (double *)malloc(sizeof(double)*N_BLOCK_ROW * N_BLOCK_COL);
...
#if TCACHE
  	flush_cache();
	volatile double * descriptor_cached= (double *)tcaching_malloc(descriptor, sizeof(double)*HISTOGRAMSIZE);
	volatile double * sum_of_block_cached= (double *)tcaching_malloc(sum_of_block,sizeof(double)*N_BLOCK_ROW*N_BLOCK_COL);
	volatile int * lut0_cached  = (int *)tcaching_malloc(lut0,sizeof(int)*256);
	volatile int * lut1_cached  = (int *)tcaching_malloc(lut1,sizeof(int)*256);
	volatile int * lut2_cached  = (int *)tcaching_malloc(lut2,sizeof(int)*256);
	volatile int * lut3_cached  = (int *)tcaching_malloc(lut3,sizeof(int)*256);
	for(int i = 0; i < HISTOGRAMSIZE; i++) descriptor_cached[i] = 0;
	for(int i = 0; i < N_BLOCK_ROW * N_BLOCK_COL; i++) sum_of_block_cached[i] = 0;
