...
int receive_float_data_msg() {
  ...
  //printf("Received XTS_FLAGSEQUENCE_START_NOESCAPE");

  // Receive the size of the package (in bytes) starting from after the <RESERVED> has been received
  uint32_t packet_length = 0;
  while (!uwb_uart_available2()) {}
  packet_length |= (uwb_uart_read2());
  while (!uwb_uart_available2()) {}
  packet_length |= (uint32_t)uwb_uart_read2() << 8;
  while (!uwb_uart_available2()) {}
  packet_length |= (uint32_t)uwb_uart_read2() << 16;
  while (!uwb_uart_available2()) {}
  packet_length |= (uint32_t)uwb_uart_read2() << 24;
